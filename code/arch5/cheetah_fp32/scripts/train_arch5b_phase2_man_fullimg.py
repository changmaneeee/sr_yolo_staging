#!/usr/bin/env python3
"""
Arch5B Phase 2 with MAN backbone -- Fusion Warm-up (FULL IMAGE)
================================================================
...
Key differences from train_arch5b_phase2_man.py:
  - SRDetPairDataset (full images, NO cropping)
  - batch_size=2 (conservative for 24GB with MAN transformer)
  - OUT_DIR = iac_lab/runs/arch5b_phase2_man_fullimg
...
OUT_DIR = PROJECT_ROOT / "iac_lab/runs/arch5b_phase2_man_fullimg"
...
def main():
    ...
    print("=" * 60)
    print("Arch5B Phase 2 -- MAN Backbone (Fusion Warm-up, FULL IMAGE)")
    ...
    cfg_path = PROJECT_ROOT / "configs/experiment/arch5b_phase2_man.yaml"
    ...
    for epoch in range(epochs):
        ...
        if va < best_val:
            ...
            torch.save(model.state_dict(), OUT_DIR / "phase2_man_fullimg_best.pt")
        ...
        if (epoch + 1) % 10 == 0:
            torch.save(model.state_dict(), OUT_DIR / f"phase2_man_fullimg_epoch{epoch+1}.pt")

    torch.save(model.state_dict(), OUT_DIR / "phase2_man_fullimg_last.pt")
    with open(OUT_DIR / "history.json", "w") as f:
        json.dump(history, f, indent=2)

    print(f"\n{'='*60}")
    print(f"PHASE 2 COMPLETE (FULL IMAGE). Best epoch {best_epoch}, val={best_val:.4f}")
    print(f"Checkpoint: {OUT_DIR / 'phase2_man_fullimg_best.pt'}")
    print(f"{'='*60}")
