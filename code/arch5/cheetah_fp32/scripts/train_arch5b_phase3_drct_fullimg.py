#!/usr/bin/env python3
"""
Arch5B Phase 3 with DRCT backbone -- Joint Training (FULL IMAGE)
================================================================
Problem: DRCT was trained on 64x64 LR crops (256x256 HR) due to OOM on 4060.
...
Key differences from train_arch5b_phase3_drct.py:
  ...
  - OUT_DIR = iac_lab/runs/arch5b_phase3_drct_fullimg
...
OUT_DIR = PROJECT_ROOT / "iac_lab/runs/arch5b_phase3_drct_fullimg"
...
def main():
    ...
    print("=" * 60)
    print("Arch5B Phase 3 -- DRCT Backbone (Joint Training, FULL IMAGE)")
    ...
    cfg_path = PROJECT_ROOT / "configs/experiment/arch5b_phase3_drct.yaml"
    ...
    phase3_ckpt = OUT_DIR / "phase3_drct_fullimg_best.pt"
    phase2_ckpt = PROJECT_ROOT / "iac_lab/runs/arch5b_phase2_drct_fullimg/phase2_drct_fullimg_best.pt"
    start_epoch = 0
    ...
    for epoch in range(start_epoch, epochs):
        ...
        if va < best_val:
            ...
            torch.save(model.state_dict(), OUT_DIR / "phase3_drct_fullimg_best.pt")
        ...
        torch.save(model.state_dict(), OUT_DIR / "phase3_drct_fullimg_last.pt")
        if (epoch+1) % 10 == 0:
            torch.save(model.state_dict(), OUT_DIR / f"phase3_drct_fullimg_epoch{epoch+1}.pt")
    ...
    print(f"Checkpoint: {OUT_DIR / 'phase3_drct_fullimg_best.pt'}")
    print(f"{'='*60}")
