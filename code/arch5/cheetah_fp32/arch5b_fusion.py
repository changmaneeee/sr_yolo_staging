...
class Arch5BFusion(BasePipeline):
    ...
    def _init_drct_sr(self, model_config):
        ...
        # gradient checkpointing은 unfreeze_for_phase3()에서 활성화 (Phase3 backward OOM 방지)
    ...
    def _init_hat_sr(self, model_config):
        """HAT SR 모델 초기화 (DRCT와 거의 동일한 패턴)"""
        from sci_lab.backbones.hat_wrapper import HATWrapper

        sr_block = get_val(model_config, 'sr', SimpleNamespace())
        hat_config = get_val(model_config, 'hat', get_val(sr_block, 'hat', SimpleNamespace()))
        weights_config = get_val(model_config, 'weights', SimpleNamespace())

        pretrain_path = get_first_val(
            hat_config, ['pretrain_path', 'weights_path'],
            get_first_val(sr_block, ['weights', 'pretrain_path'],
                          get_val(weights_config, 'sr_model', None))
        )
        variant = get_val(hat_config, 'variant', 'base')
        ...
        if pretrain_path:
            ...
            if 'model_state_dict' in ckpt:
                # Fine-tuned checkpoint from train_sr_only.py
                state = ckpt['model_state_dict']
                # Keys have 'model.' prefix from HATWrapper
                self.sr_model.load_state_dict(state, strict=False)
                psnr = ckpt.get('psnr', '?')
                print(f"[Arch5B] HAT fine-tuned 로드: {pretrain_path} (PSNR={psnr})")
            elif 'params_ema' in ckpt:
                self.sr_model.model.load_state_dict(ckpt['params_ema'], strict=False)
                print(f"[Arch5B] HAT pretrained 로드: {pretrain_path}")
            elif 'params' in ckpt:
                self.sr_model.model.load_state_dict(ckpt['params'], strict=False)
                print(f"[Arch5B] HAT pretrained 로드: {pretrain_path}")
            else:
                self.sr_model.model.load_state_dict(ckpt, strict=False)
                print(f"[Arch5B] HAT raw weights 로드: {pretrain_path}")

        self.sr_feature_channels = self.sr_model.feature_channels
        print(f"[Arch5B] HAT variant={variant}, feature_channels={self.sr_feature_channels}")

    def _init_man_sr(self, model_config):
        """MAN SR 모델 초기화 (DRCT와 거의 동일한 패턴, n_feats=180)"""
        from sci_lab.backbones.man_wrapper import MANWrapper

        sr_block = get_val(model_config, 'sr', SimpleNamespace())
        man_config = get_val(model_config, 'man', get_val(sr_block, 'man', SimpleNamespace()))
        weights_config = get_val(model_config, 'weights', SimpleNamespace())

        pretrain_path = get_first_val(
            man_config, ['pretrain_path', 'weights_path'],
            get_first_val(sr_block, ['weights', 'pretrain_path'],
                          get_val(weights_config, 'sr_model', None))
        )
        variant = get_val(man_config, 'variant', 'base')
        ...
        if pretrain_path:
            ...
            if 'model_state_dict' in ckpt:
                # Fine-tuned checkpoint from train_sr_only.py
                state = ckpt['model_state_dict']
                # Keys have 'model.' prefix from MANWrapper
                self.sr_model.load_state_dict(state, strict=False)
                psnr = ckpt.get('psnr', '?')
                print(f"[Arch5B] MAN fine-tuned 로드: {pretrain_path} (PSNR={psnr})")
            elif 'params_ema' in ckpt:
                self.sr_model.model.load_state_dict(ckpt['params_ema'], strict=False)
                print(f"[Arch5B] MAN pretrained 로드: {pretrain_path}")
            elif 'params' in ckpt:
                self.sr_model.model.load_state_dict(ckpt['params'], strict=False)
                print(f"[Arch5B] MAN pretrained 로드: {pretrain_path}")
            else:
                self.sr_model.model.load_state_dict(ckpt, strict=False)
                print(f"[Arch5B] MAN raw weights 로드: {pretrain_path}")

        self.sr_feature_channels = self.sr_model.feature_channels
        print(f"[Arch5B] MAN variant={variant}, feature_channels={self.sr_feature_channels}")
    ...
    def forward(
        ...
        """
        # 1. SR Feature 추출
        # ⚠️ SR backbone은 Phase 2에서 frozen이며, HAT/DRCT 같은 transformer는
        #    fp16 AMP autocast에서 attention softmax overflow → NaN 발생 (2026-05-28 HAT 실패).
        #    SR은 항상 fp32로 실행 (frozen이라 메모리 부담 작음). Fusion/YOLO는 fp16 유지.
        with torch.cuda.amp.autocast(enabled=False):
            lr_fp32 = lr_image.float()
            if self.sr_type == 'mamba':
                sr_features = self.sr_model.encode(lr_fp32)
            else:
                sr_features = self.sr_model.forward_features(lr_fp32)
        ...
        if return_features:
            # ⭐ [최적화] SR image도 미리 계산하여 캐싱 (SR은 fp32 강제 — NaN 방지)
            if sr_image is None:
                with torch.cuda.amp.autocast(enabled=False):
                    if self.sr_type == 'mamba':
                        sr_image = self.sr_model.decode(sr_features)
...
# =========================================================================
    ...
    def unfreeze_for_phase3(self) -> Dict[str, List]:
        ...
        # ⚠️ Phase 3에서 SR이 unfreeze되면 backward activation이 폭증해
        #    full image(192→768) fp32에서 24GB OOM 발생.
        #    gradient checkpointing 적용 → 메모리 대폭↓ (결과 수학적 동일, 속도만 20-30%↓).
        #    HAT/DRCT(transformer) + MAN(real batch 2에서 24GB 도달, 2026-06-02 확인) 모두 적용.
        if self.sr_type in ('hat', 'drct', 'man') and hasattr(self.sr_model, 'enable_gradient_checkpointing'):
            self.sr_model.enable_gradient_checkpointing()
